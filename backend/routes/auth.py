...
def register():
    ...
    try:
        ...
        if not college_email.endswith('@srmist.edu.in'):
            return jsonify({'error': 'College email must end with @srmist.edu.in'}), 400
        ...
        if role == 'student' and batch_id:
            ...
            return jsonify({'error': 'Batch selection is required for students'}), 400
        ...
        try:
            ...
            if not user or not user.check_password(password):
                ...
                if is_locked:
                    return jsonify({
                        'error': 'Too many failed login attempts. Account temporarily locked.',
                        'lockout_remaining': brute_force.is_locked(client_id)[1] or 900,
