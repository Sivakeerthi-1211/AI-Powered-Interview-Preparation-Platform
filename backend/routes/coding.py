...
def get_question(question_id):
    ...
    IMPORTANT: Hide test cases from students (LeetCode-style)
    Only faculty/admin can see all test cases
    """
    try:
        ...
        return jsonify({
            'question': question.to_dict(hide_test_cases=hide_test_cases)
        }), 200
...
def submit():
    ...
    try:
        ...
        # 1. Comprehensive code analysis (AST, complexity, memory, quality, best practices)
        comprehensive_analysis = analyze_code_comprehensive(
            code=code,
            language=language,
            question_description=question_description
        ...
        # 2. Generate additional test cases (normal, edge, extreme, invalid, empty)
        generated_test_cases = TestCaseGenerator.generate(question_description, language)
        
        # 3. Execute against original test cases (hidden)
        execution_result = execute_submit_mode(code, language, test_cases)
        
        # 4. Multi-Test Engine: Execute against ALL test cases (normal, edge, extreme, invalid, empty)
        ...
        # Extract generated test results for backward compatibility
        generated_test_results = [
            r for r in multi_test_results['results']
            if r['test_type'] != 'original'
        ]
        
        # 5. Legacy code intelligence (for backward compatibility)
        code_intelligence = analyze_code_intelligence(code, language)
        
        # 6. Generate comprehensive interview-style feedback
