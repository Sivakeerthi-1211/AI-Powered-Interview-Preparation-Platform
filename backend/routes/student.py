...
def ai_chat():
    ...
    try:
        ...
        print(f"[AI Chat Debug] API Key present: {bool(openai_api_key)}")
        print(f"[AI Chat Debug] API Key starts with: {openai_api_key[:15] if openai_api_key else 'None'}...")
        print(f"[AI Chat Debug] Model: {openai_model}")
        print(f"[AI Chat Debug] User message: {message[:50]}...")
        print(f"[AI Chat Cost-Saving] Message length: {message_length}, Max tokens: {max_tokens}")
        ...
        if openai_api_key:
            try:
                ...
                print(f"[AI Chat Debug] Sending request to OpenAI with {len(messages)} messages...")
                ...
                print(f"[AI Chat Debug] OpenAI response received: {ai_response[:50]}...")
                ...
                if 'quota' in error_str.lower() or '429' in error_str or 'insufficient_quota' in error_str.lower():
                    print("[AI Chat Error] API Key quota exceeded. Please check your OpenAI account billing.")
                    # Return a helpful message about quota
                    ai_response = "I'm currently unable to process requests due to API quota limits. Please check your OpenAI account billing or contact support."
                elif 'invalid' in error_str.lower() or '401' in error_str or 'unauthorized' in error_str.lower():
                    print("[AI Chat Error] Invalid API Key. Please update OPENAI_API_KEY in config.py")
                    ai_response = "API configuration issue. Please contact support."
                else:
                    # Set to None to trigger generic fallback
                    ai_response = None
        
        # If no API key or API failed, return generic message
        if not ai_response:
            print(f"[AI Chat Debug] No response generated. API Key present: {bool(openai_api_key)}")
            if not openai_api_key:
                print("[AI Chat Debug] ERROR: No API key found in config!")
            ai_response = "I'm here to help! Could you please try asking your question again?"
        
        print(f"[AI Chat Debug] Final response length: {len(ai_response) if ai_response else 0}")
        print(f"[AI Chat Debug] Final response preview: {ai_response[:100] if ai_response else 'None'}...")
