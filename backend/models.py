...
class User(db.Model):
    ...
    college_email = db.Column(db.String(120), unique=True, nullable=False, index=True)  # Must end with @srmist.edu.in
...
class Question(db.Model):
    ...
    def to_dict(self, hide_test_cases=False):
        ...
        hide_test_cases: If True, hide test cases from students (LeetCode-style)
        """
        data = {
            'id': self.id,
            'title': self.title,
        ...
        if self.type == 'coding':
            ...
            if hide_test_cases:
                # Only show sample test cases (first one) for students
                test_cases = json.loads(self.test_cases) if self.test_cases else []
                if test_cases:
                    # Show only first test case as example
                    data['sample_test_cases'] = [test_cases[0]] if len(test_cases) > 0 else []
                else:
                    data['sample_test_cases'] = []
                # Do NOT expose all test cases
            else:
                # Faculty/Admin can see all test cases
...
class ChatHistory(db.Model):
    ...
    user = db.relationship('User', backref='chat_history', lazy=True)
...
class UsageStats(db.Model):
    ...
    user = db.relationship('User', backref='usage_stats', lazy=True)
