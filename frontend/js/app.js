...
async function testBackendConnection() {
    try {
        ...
        if (response.ok) {
            ...
            console.log('[Connection] ✅ Backend API connected:', data.message || 'OK');
            return true;
        } else {
            console.warn('[Connection] ⚠️ Backend API returned status:', response.status);
            return false;
        }
    } catch (error) {
        if (error.name === 'AbortError') {
            console.error('[Connection] ❌ Backend connection test timed out (5s)');
        } else {
            console.error('[Connection] ❌ Cannot connect to backend API:', error.message);
        }
        console.error('[Connection] 💡 Make sure backend server is running:');
...
function checkAuth() {
    ...
    if (authToken && currentUser) {
        ...
        showAuthPage();
        // Clear any hash if not logged in
...
function showMainApp() {
    ...
    if (authPage) {
        authPage.classList.remove('active');
        authPage.style.display = 'none';
        console.log('[showMainApp] ✅ Hiding auth page (removed active class)');
    }
    if (mainApp) {
        mainApp.classList.add('active');
        mainApp.style.display = 'block';
        console.log('[showMainApp] ✅ Showing main app (added active class)');
    }
    
    // Then trigger renderer to sync state
    if (typeof render === 'function') {
        render();
        console.log('[showMainApp] ✅ Renderer called');
    }
...
async function login() {
    ...
    try {
        ...
        const loginResponse = await authAPI.login(username, password, { timeout: 10000 });
...
async function register() {
    ...
    // Validate college email (must end with @srmist.edu.in)
    if (!collegeEmail.endsWith('@srmist.edu.in')) {
        errorDiv.textContent = 'College email must end with @srmist.edu.in';
        errorDiv.style.display = 'block';
        return;
    }
...
function checkPasswordMatch() {
    ...
    if (password === confirmPassword) {
        matchIndicator.innerHTML = '<span style="color: #2ecc71;">✓ Passwords match</span>';
        matchIndicator.style.color = '#2ecc71';
    } else {
        matchIndicator.innerHTML = '<span style="color: #e74c3c;">✗ Passwords do not match</span>';
...
function handleBatchBothCheckbox(checkbox) {
    // When "Both Batches" is checked, check both Batch 1 and Batch 2
    if (checkbox.checked) {
        const batch1Checkbox = document.querySelector('input[name="assigned-batches"][value="1"]');
        const batch2Checkbox = document.querySelector('input[name="assigned-batches"][value="2"]');
        if (batch1Checkbox) batch1Checkbox.checked = true;
        if (batch2Checkbox) batch2Checkbox.checked = true;
    }
}

function handleBatchCheckboxChange() {
    // When Batch 1 or Batch 2 changes, update "Both Batches" checkbox
    const batch1Checkbox = document.querySelector('input[name="assigned-batches"][value="1"]');
    const batch2Checkbox = document.querySelector('input[name="assigned-batches"][value="2"]');
    const bothCheckbox = document.getElementById('batch-both-checkbox');
    
    if (batch1Checkbox && batch2Checkbox && bothCheckbox) {
        bothCheckbox.checked = batch1Checkbox.checked && batch2Checkbox.checked;
    }
}
...
async function submitAssessmentStep1(event) {
    ...
    try {
        ...
        const submitBtn = event.target.querySelector('button[type="submit"]');
        submitBtn.disabled = false;
        submitBtn.textContent = 'Save & Continue to Step 2 →';
    }
}
    ...
    function initProctoringWithRetry(maxRetries = 10, delay = 200) {
        ...
        function checkAndInit() {
            ...
            if (typeof initializeProctoring === 'function') {
                console.log('[Interview] ✅ Proctoring available. Initializing...');
                try {
                    initializeProctoring();
                    console.log('[Interview] ✅ Proctoring initialized successfully');
                } catch (error) {
                    console.error('[Interview] ❌ Error initializing proctoring:', error);
                }
                return true;
            }
            ...
            if (retries < maxRetries) {
                ...
                console.error('[Interview] ❌ Proctoring script not loaded after', maxRetries, 'attempts');
    ...
    try {
        ...
        if (user.role === 'faculty' || user.role === 'admin') {
            ...
            if (facultyDashboard) {
                ...
                console.log('[Dashboard] ✅ Faculty/Admin dashboard shown');
            } else {
                console.error('[Dashboard] ❌ Faculty/Admin dashboard element not found');
            }
        ...
        if (studentDashboard) {
            ...
            console.log('[Dashboard] ✅ Student dashboard shown');
        } else {
            console.error('[Dashboard] ❌ Student dashboard element not found');
        }
        ...
        console.log('[Dashboard] ✅ Student dashboard data loaded');
    } catch (error) {
...
async function loadFacultyAdminDashboard() {
    try {
        ...
        if (facultyDashboard) {
            ...
            console.error('[Faculty/Admin Dashboard] ❌ Dashboard element not found');
            return;
        }
        ...
        try {
            ...
            console.log('[Faculty/Admin Dashboard] ✅ Dashboard data loaded:', dashboardData);
        } catch (apiError) {
        ...
        console.log('[Faculty/Admin Dashboard] ✅ Dashboard fully loaded');
    } catch (error) {
...
async function loadDailyStreaks() {
    try {
        ...
        if (codingStreakEl && data.coding_streak >= 7) {
            codingStreakEl.textContent = `🔥 ${data.coding_streak}`;
        }
        if (quizStreakEl && data.quiz_streak >= 7) {
            quizStreakEl.textContent = `🔥 ${data.quiz_streak}`;
        }
        if (interviewStreakEl && data.interview_streak >= 7) {
            interviewStreakEl.textContent = `🔥 ${data.interview_streak}`;
        }
    } catch (error) {
...
function renderStudentDetails(details, studentName) {
    ...
    content.innerHTML = `
        <div class="student-details-header">
            <h2>📊 ${studentName}'s Performance Details</h2>
            <button class="close-modal-btn" onclick="closeStudentDetailsModal()">×</button>
        </div>
...
async function exportStudentPerformancePDF() {
    ...
    try {
        ...
        if (option) {
            ...
            option.innerHTML = '<span>⏳</span> Generating PDF...';
        }
        ...
        if (option) {
            ...
            option.innerHTML = '<span>📄</span> Student Performance (PDF)';
        }
    } catch (error) {
...
async function exportBatchAnalyticsCSV() {
    ...
    try {
        ...
        if (option) {
            option.disabled = true;
            option.innerHTML = '<span>⏳</span> Generating CSV...';
        }
        ...
        if (option) {
            option.disabled = false;
            option.innerHTML = '<span>📊</span> Batch Analytics (CSV)';
        }
    } catch (error) {
...
async function submitStudentFeedback(event, studentId, studentName) {
    ...
    try {
        ...
        submitBtn.disabled = false;
        submitBtn.innerHTML = '<span>📤</span><span>Send Feedback</span>';
...
async function openCodingQuestion(questionId, updateHash = true) {
    ...
    try {
        ...
        // Get test cases - check both test_cases (for faculty/admin) and sample_test_cases (for students)
        let testCases = [];
        try {
            if (question.test_cases) {
                // Faculty/Admin see all test cases
                testCases = typeof question.test_cases === 'string' ? JSON.parse(question.test_cases) : question.test_cases;
            } else if (question.sample_test_cases) {
                // Students see only sample test cases
        ...
        // Determine if user is student (to show appropriate message)
        const isStudent = currentUser && currentUser.role === 'student';
        const testCasesLabel = isStudent ? 'Sample Test Cases' : 'Test Cases';
        ...
        let testCasesHTML = '';
        if (testCases.length > 0) {
            const studentNote = isStudent ? '<p class="leetcode-test-note">These are sample test cases. Your code will be tested against additional hidden test cases when you submit.</p>' : '';
...
function renderCompanies() {
    ...
    companiesDiv.innerHTML = filteredCompanies.map(c => `
        <div class="company-card ${companyDeleteMode ? 'delete-mode' : ''}" data-company-id="${c.id}">
            ...
            ${!companyDeleteMode ? `
                <button class="btn btn-sm" onclick="openCompanyQuestions(${c.id}); event.stopPropagation();" 
                ...
                <div class="post-card">
                    ...
                    ${post.content ? `
                        <div class="post-description-section" style="margin: 15px 0; padding: 15px; background: rgba(255,255,255,0.03); border-radius: 8px; border-left: 3px solid rgba(102, 126, 234, 0.5);">
                            <h5 style="color: #8b9aff; margin-bottom: 10px; font-size: 14px;">📝 Description / Topics</h5>
...
function showCompanyPostsPage(companyId, companyName) {
    ...
    // Store current company ID for reference
...
function updatePostOptionsContainer() {
    ...
    while (currentRowCount < postOptionCount) {
        ...
        row.innerHTML = `
            <input type="text" class="option-input" placeholder="Option ${letter}" data-option="${letter}">
            <button class="btn-remove-option" onclick="removePostOption(this)">✕</button>
        `;
        container.appendChild(row);
        currentRowCount++;
    }
            ...
            tableBody.innerHTML = data.users.map((user, index) => {
                ...
                const batchDropdown = user.role === 'student' ? `
                    <select id="batch-select-${user.id}" onchange="updateUserBatch(${user.id}, this.value)" 
                        ...
                        <option value="1" ${user.batch_id == 1 ? 'selected' : ''} style="background: white; color: #333;">Batch 1</option>
                        <option value="2" ${user.batch_id == 2 ? 'selected' : ''} style="background: white; color: #333;">Batch 2</option>
                    </select>
        ...
        <div style="background: #1a1a2e; padding: 30px; border-radius: 8px; max-width: 400px; width: 90%; border: 1px solid #444;">
            ...
            <div style="margin-bottom: 15px;" id="edit-batch-container" style="display: ${currentRole === 'student' ? 'block' : 'none'};">
                ...
                <select id="edit-batch-select" style="width: 100%; padding: 8px; border: 1px solid #555; border-radius: 4px; background: #2a2a3e; color: white;">
                    ...
                    <option value="1" ${currentBatchId == 1 ? 'selected' : ''} style="background: #2a2a3e; color: white;">Batch 1</option>
                    <option value="2" ${currentBatchId == 2 ? 'selected' : ''} style="background: #2a2a3e; color: white;">Batch 2</option>
                </select>
            </div>
...
async function updateUserBatch(userId, batchId) {
    ...
    const batchName = batchId === '1' ? 'Batch 1' : batchId === '2' ? 'Batch 2' : 'No Batch';
...
async function editAssessment(assessmentId) {
    try {
        ...
        if (assessment.assigned_batches && Array.isArray(assessment.assigned_batches)) {
            if (batch1Checkbox) batch1Checkbox.checked = assessment.assigned_batches.includes(1);
            if (batch2Checkbox) batch2Checkbox.checked = assessment.assigned_batches.includes(2);
            if (bothCheckbox) bothCheckbox.checked = assessment.assigned_batches.includes(1) && assessment.assigned_batches.includes(2);
        } else {
            // NULL means all batches - uncheck all
            if (batch1Checkbox) batch1Checkbox.checked = false;
            if (batch2Checkbox) batch2Checkbox.checked = false;
