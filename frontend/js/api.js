...
// Get API URL from index.html script tag, or fallback to localhost
const getApiBaseUrl = () => {
...
async function apiRequest(endpoint, options = {}) {
    ...
    // Use token manager if available (for auto-refresh)
...
const authAPI = {
    login: async (username, password, options = {}) => {
        const data = await apiRequest('/auth/login', {
            ...
            timeout: options.timeout || 30000 // 30 seconds for login (increased for slow connections)
        });
